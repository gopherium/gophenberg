...
func TestServedKitsNamesWhatThisReleaseServes(t *testing.T) {
	...
	if !slices.Equal(served, []string{"0.9.0", "0.10.0", "0.11.0", "0.12.0", "0.13.0", "0.14.0"}) {
		t.Errorf("ServedKits() = %v, want the kit versions this release serves", served)
	}
	if themehost.NewestKit() != "0.14.0" {
...
func TestAThemeOnEitherServedKitIsAccepted(t *testing.T) {
	...
	for _, declared := range []string{"0.9.0", "0.10.0", "0.11.0", "0.12.0", "0.13.0", "0.14.0"} {
