...
func TestPostDetailReportsAuthorLookupFailures(t *testing.T) {
	...
	handler := authedServerWithStores(t, server.Config{Users: failingUserStore{Store: users}, Posts: posts})
...
func TestPostListAcceptsAnUnregisteredType(t *testing.T) {
	...
	list := doRequest(t, handler, http.MethodGet, "/api/posts?type=page", "")
	counts := doRequest(t, handler, http.MethodGet, "/api/posts/counts?type=page", "")
...
func TestPostErrorsFallBackToTheAuthMapping(t *testing.T) {
	...
	handler := authedServerWithStores(t, server.Config{Users: users, Posts: posts})

	recorder := doRequest(t, handler, http.MethodPost, "/api/posts", `{"title":"Hi"}`)
